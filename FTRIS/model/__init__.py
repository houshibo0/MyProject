...
def build_segmenter(args):
    ...
    logger.info(f'number of updated params (Backbone): {n_backbone_parameters} = {n_backbone_parameters/1e6:.3f} M')
    n_head_parameters = sum(p.numel() for p in head)
    logger.info(f'number of updated params (Head)    : {n_head_parameters} = {n_head_parameters/1e6:.3f} M')
    n_fixed_parameters = sum(p.numel() for p in fix)
    logger.info(f'number of fixed params             : {n_fixed_parameters} = {n_fixed_parameters/1e6:.3f} M')
